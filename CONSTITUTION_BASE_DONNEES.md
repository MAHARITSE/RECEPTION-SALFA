...
| `prescriptions` | `HbLine[]` (anciennement `Prescription[]`) | Lignes d'ordonnance (fusionnées dans HbLine) |
...
## 📋 4. ~~TABLE `Prescription`~~ → **SUPPRIMÉE — fusionnée dans `HbLine`**

> **Les lignes de prescription sont désormais gérées directement via `HbLine`.**
> L'ancienne interface `Prescription` est conservée comme alias de compatibilité (`type Prescription = HbLine`)
> et sera retirée dans une prochaine passe.
>
> Toutes les colonnes de l'ancienne table `Prescription` (id, articleId, articleName, family, quantity,
> posology, duration, instructions, unitPrice, discount, delivered, deliveredAt, dateSort, venteLineId)
> existent déjà dans `HbLine` (voir section 12 ci-dessous).
>
> La collection `Consultation.prescriptions` utilise désormais directement le type `HbLine[]`.
...
> **Fusion fonctionnelle complète :** `HbLine` est désormais le type **unique** pour toutes les lignes d'articles — prescriptions d'ordonnance, lignes Hospitalisation / Bloc, ventes comptoir et ventes externes. L'ancienne table `Prescription` a été supprimée et fusionnée ici.
...
> `ArticleFamily` est un `string` : la famille d'un article référence le champ `code` d'une ligne `familles`. Les codes par défaut (`MEDIC`, `LABO`, `DENT`, `ECHO`) restent supportés, mais les familles créées dans la base sont utilisables dans la fiche article, les lignes HbLine (ordonnance + hospit/bloc) et les ventes.
...
### Besoin fonctionnel
Les écrans de saisie **Médecin (ordonnance)**, **Hospitalisation** et **Bloc** utilisent le même type `HbLine` et les lignes convergent vers `venteLines` :

- la famille de l'article est copiée depuis la fiche article (`Article.family`) ;
- toutes les lignes (ordonnance, Hospitalisation, Bloc, comptoir, externe) utilisent **uniquement** `HbLine` ;
- la date de sortie (`dateSort`) est conservée en base ;
- pour une ordonnance, `dateSort` est alimentée automatiquement avec la **date de délivrance pharmacie** (`deliveredAt`).

Dans la saisie Sage (Médecin / Hospitalisation / Bloc), la date est affichée immédiatement **après le montant**. Elle permet de distinguer :
...
5. **Ordonnance** : à l'encaissement, une ligne d'ordonnance (HbLine) crée une `venteLine` avec `prescriptionId`; la date `dateSort` est complétée lors de la délivrance pharmacie.
...
| `prescriptionId` | `string?` | FK logique → HbLine.id (ancienne Prescription) |
...
| `dateSort` | `string?` | 📅 Date d'acte / de sortie ; pour les ordonnances = date de délivrance pharmacie |
