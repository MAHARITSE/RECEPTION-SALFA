...
- **Fiche client** (double-clic sur un dossier) : liste **toutes** ses factures, toutes dates confondues, avec son propre champ de **recherche** (n° facture, date, article, montant — sans filtre de mois, utile quand un client externe a beaucoup de factures). Les factures cochées s'impriment **2 par page A4** — **deux en-têtes séparés**, un par facture, chacun cantonné à sa moitié de feuille — ou se **fusionnent en une seule facture** (bouton **Fusionner**, A4 paysage : contenu à gauche, suite sur la moitié droite de la feuille) qui porte le **numéro de la plus ancienne** des factures cochées **sans lister** les factures d'origine (articles mis à la suite).
