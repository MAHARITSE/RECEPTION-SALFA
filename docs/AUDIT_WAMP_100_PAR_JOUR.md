...
| Sécurité — exposition WAMP | 🔴 **Critique, hors application** | MySQL de WAMP écoute sur le LAN avec `root` **sans mot de passe** ; le fichier d'installation SQL (comptes par défaut) téléchargeable si `AllowOverride` inactif. |
...
| S3 | **P0** | **Mots de passe par défaut connus** (`admin123`, `caisse123`…) présents dans le fichier d'installation `database/reception_salfa_complete.sql` (ex-`seed.sql`), donc dans le dépôt **et** dans le dossier déployé. Si `AllowOverride` est inactif, `GET /reception-salfa/database/reception_salfa_complete.sql` renvoie le fichier → connexion immédiate en admin. | `reception_salfa_complete.sql`, `.htaccess` inefficace si Apache ne l'applique pas | 🟠 À faire (checklist §5) + 🟢 `429` après 5 échecs (S4) et `.htaccess` durci avec vérification `curl` documentée. |
...
| S12 *(corrigé)* | P2 | **Sauvegarde SQL de l'application non réimportable** : `buildSqlBackup()` crée des tables `invoices`, `stockEntries`, `pharmaDeliveryItems`… avec une colonne `LONGTEXT`, alors que le schéma réel est `factures`, `entrees_stock`, `livraisons_pharmacie` avec `donnees JSON` + `numero_ref`. Le dump « Sauvegarder » s'importe donc dans une base **parallèle que l'application ne lit pas**. | `src/utils/sauvegardeSql.ts:34-68` vs `database/reception_salfa_complete.sql` | 🔴 **À corriger** : soit mapper les noms + colonnes vers le schéma v3, soit retirer ce bouton au profit de `outils/sauvegarder.bat` (mysqldump, lui, est correct). ✅ **Corrigé** : `src/utils/sauvegardeSql.ts` est réécrit en miroir exact de
  `salfa_tables()` (35 datasets, noms de tables français, `donnees` JSON,
  `numero_ref`/`dossier_ref`) ; DDL `IF NOT EXISTS` identique à `reception_salfa_complete.sql` ;
  `INSERT … ON DUPLICATE KEY UPDATE` par paquets de 600 Ko (`max_allowed_packet`) ;
  `journal_audit` en `INSERT IGNORE` (un fichier ne peut pas réécrire l'historique) ;
  `compteurs` en `GREATEST` (jamais décrémentés) ; `parametres` pour
  `ticketSettings`/`monthlyInvoices`/`issuedFactureNumbers`/`prescripteursExternes`
  (jamais exportés jusqu'ici) ; mots de passe en clair filtrés, hachage serveur
  conservé mais **jamais écrasé** à l'import (`JSON_SET` + `COALESCE`) ;
  refus au-delà de 150 Mo avec renvoi vers `outils/sauvegarder.bat`. Le fichier est
  téléchargé par le bouton *Sauvegarder*, par Administration → Données, et
  automatiquement à la déconnexion si la journée n'a rien produit. |
...
5. Vérifier les protections Apache :
   ```bat
   curl -i http://SERVEUR/reception-salfa/database/reception_salfa_complete.sql      :: attendu 403
