...
const users: User[] = [
  ...
  ['USR-CASH', 'Caisse 1 - Miora Kanto', 'cashier', 'caisse123'],
  ['USR-CASH2', 'Caisse 2 - Pierre Duval', 'cashier', 'caisse123'],
  ['USR-PHA', 'Pharmacie 1 - Tiana Soa', 'pharmacy', 'pharma123'],
  ['USR-PHA2', 'Pharmacie 2 - Fatima Benali', 'pharmacy', 'pharma123'],
...
export function createMassiveDemoState(): AppState {
  ...
  return {
    ...
    auditLogs: [],
    notifications: [
      { id: id('notification', 0), targetRole: 'pharmacy' as const, message: '🚨 Rupture de stock (Pharmacie) : Amoxicilline 1g', type: 'critical' as const, timestamp: iso(0), read: false },
      { id: id('notification', 1), targetRole: 'magasinier' as const, message: '⚠️ Stock bas central : Paracétamol 500mg (45 / alerte: 100)', type: 'warning' as const, timestamp: iso(1), read: false },
      { id: id('notification', 2), targetRole: 'magasinier' as const, message: "📩 [Pharmacie] Demande d'approvisionnement : 3 article(s)", type: 'info' as const, timestamp: iso(2), read: false },
      { id: id('notification', 3), targetRole: 'pharmacy' as const, message: '📦 Réapprovisionnement reçu du dépôt central : Ibuprofène 400mg (100 boîtes)', type: 'info' as const, timestamp: iso(3), read: true },
      { id: id('notification', 4), targetRole: 'pharmacy' as const, message: '⚠️ Stock bas pharmacie : Oméprazole 20mg (15 / alerte: 30)', type: 'warning' as const, timestamp: iso(4), read: true },
      { id: id('notification', 5), targetRole: 'magasinier' as const, message: '🚨 Rupture de stock central : Gel échographie', type: 'critical' as const, timestamp: iso(5), read: true },
    ],
