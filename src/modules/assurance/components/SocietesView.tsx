...
import { Building, Plus, Search, Edit3, Trash2, Phone, Mail, MapPin, X, Layers, GitMerge, CheckSquare, Square, ArrowRight, Check, Sparkles, RefreshCw, ShieldOff, Ban, Users, Percent } from 'lucide-react';
...
import { societeNatureRemise } from '../utils/natureRemise';
import { NATURES_REMISE, natureRemiseBadge, natureRemiseOuDefaut } from '../../../utils/natureRemise';
...
export const SocietesView: React.FC<SocietesViewProps> = ({
  ...
  const [formData, setFormData] = useState<Partial<Societe>>({
    ...
    natureRemise: 'ticket_moderateur',
    exclusions: [],
    blacklisted: false,
    blacklistReason: '',
    blacklistUntil: '',
  ...
  const handleOpenEdit = (s: Societe) => {
    ...
    setFormData({
      ...
      natureRemise: societeNatureRemise(s),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const toSave: Societe = {
      ...
      natureRemise: natureRemiseOuDefaut(formData.natureRemise),
    ...
    <div id="societes-view" className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map(soc => {
          ...
          // Nature de la réduction (brut − net) de la société et assurés qui y dérogent.
          const natureSoc = societeNatureRemise(soc);
          const derogations = personnes.filter(p =>
            (p.societeId || '').toLowerCase().trim() === socIdLower
            && p.natureRemise
            && p.natureRemise !== natureSoc);
            ...
            <div key={soc.id} className="bg-surface rounded-xl border border-line shadow-xs p-5 flex flex-col justify-between space-y-4 hover:border-indigo-300 transition">
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${natureRemiseBadge(natureSoc)}`}
                        title={natureSoc === 'remise'
                          ? 'Réduction = VRAIE REMISE accordée sur le prix : elle n’est due ni par l’assuré, ni par la société'
                          : 'Réduction = TICKET MODÉRATEUR : quote-part restant à la charge de l’assuré (réglage par défaut)'}
                      >
                        {natureSoc === 'remise' ? '% Remise' : 'Ticket modérateur'}
                      </span>
                    </div>
                ...
                <div className="space-y-2 text-xs text-ink-secondary pt-2 border-t border-line-soft">
                  ...
                  <div className="flex items-center justify-between bg-surface-muted p-2 rounded-lg">
                    <span className="text-ink-muted" title="Nature de la différence entre le total brut et le net à payer">Réduction facturée :</span>
                    <span className={`font-bold ${natureSoc === 'remise' ? 'text-emerald-700' : 'text-amber-700'}`}>
                      {natureSoc === 'remise' ? 'Remise accordée' : 'Ticket modérateur'}
                    </span>
                  </div>

                  {derogations.length > 0 && (() => {
                    const enRemise = derogations.filter(p => p.natureRemise === 'remise').length;
                    const enTicket = derogations.length - enRemise;
                    return (
                      <div className="flex items-start space-x-1.5 bg-indigo-50 border border-indigo-100 p-2 rounded-lg">
                        <Users className="w-3.5 h-3.5 text-indigo-500 shrink-0 mt-0.5" />
                        <p className="text-[10px] text-indigo-800">
                          <strong>{derogations.length} assuré{derogations.length > 1 ? 's' : ''}</strong> ne sui{derogations.length > 1 ? 'vent' : 't'} pas ce réglage
                          {enRemise > 0 && <> — {enRemise} en <strong>remise</strong></>}
                          {enRemise > 0 && enTicket > 0 && ' · '}
                          {enTicket > 0 && <> {enTicket} en <strong>ticket modérateur</strong></>}
                          {' '}({derogations.slice(0, 3).map(p => p.nomPrenom).join(', ')}{derogations.length > 3 ? '…' : ''}).
                        </p>
                      </div>
                    );
                  })()}
