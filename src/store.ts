...
import { attribuerNumerosFacture } from './utils/numeros';
 ...
 *
 * @deprecated Réservé aux traitements locaux mono-fil (migration du stock
 * d'anciennes factures, repli de `createVente`). Tout ÉCRAN qui émet une
 * facture doit utiliser `allocateFactureNumbersAsync` : sans l'allocation
 * atomique côté stockage, deux caisses obtiennent le même numéro.
 */
...
export interface FactureNumberSpec {
...
/**
 * Attribue UN LOT de numéros officiels, ATOMIQUEMENT côté stockage
 * (transaction IndexedDB multi-onglets / MySQL `SELECT … FOR UPDATE`
 * multi-caisses) : un seul appel pour tout le lot, numéros uniques garantis.
 * JETTE une erreur bloquante si l'allocation échoue — l'appelant doit
 * alerter et NE JAMAIS facturer sans numéro réservé.
 */
export async function allocateFactureNumbersAsync(
  state: AppState,
  specs: FactureNumberSpec[],
): Promise<FactureNumberAllocation[]> {
  ...
  const resolved = specs.map((opts) => {
    ...
    if (opts.clientType === 'societe' && (opts.company || '').trim()) {
      ...
      const parsedPresc = opts.prescriptionDate ? new Date(opts.prescriptionDate) : invoiceDate;
      const prescDate = Number.isFinite(parsedPresc.getTime()) ? parsedPresc : invoiceDate;
      return { kind: 'societe' as const, date: prescDate, code: code.code, societeUpsert: code.societeUpsert };
    }
    return {
      kind: 'standard' as const, date: invoiceDate,
      code: undefined as string | undefined, societeUpsert: undefined as Societe | undefined,
    };
  });
  const numeros = await attribuerNumerosFacture(resolved, collectExistingFactureNumbers(state));
  return resolved.map((r, i) => ({ numeroFacture: numeros[i], societeUpsert: r.societeUpsert }));
}

/**
 * Attribue UN numéro officiel (cas courant : une seule facture émise).
 * JETTE une erreur bloquante si l'allocation échoue.
 */
export async function allocateFactureNumberAsync(
  state: AppState,
  opts: FactureNumberSpec,
): Promise<FactureNumberAllocation> {
  const [alloc] = await allocateFactureNumbersAsync(state, [opts]);
  return alloc;
}
...
export function createVente(
  ...
  if (!numeroFacture) {
    ...
    // seul traitement à la fois, et amorçage depuis l'historique complet.
...
export function addJourneyEvent(
  ...
  // Nouveau tableau (jamais de mutation en place) — voir addAuditLog.
  s.journey = [ev, ...(s.journey || [])];
  return ev;
}
