...
import type { StockEntry, TransferCategory } from '../types';
import type { AppState } from '../store';
import { addAuditLog, addNotification, formatAr, ARTICLE_FAMILIES, familyLabel, transferCategoryLabel, transferCategoryColor, TRANSFER_CATEGORIES } from '../store';
import { Package, PackageCheck, PackagePlus, Search, Truck, Plus, Trash2, Save, Check, Filter, X } from 'lucide-react';
...
export default function MagasinierModule({ state, setState }: Props) {
  ...
  const [entryCatFilter, setEntryCatFilter] = useState<TransferCategory | 'all'>('all');
  const [reqCatFilter, setReqCatFilter] = useState<TransferCategory | 'all'>('all');
  const [purchaseCategory, setPurchaseCategory] = useState<TransferCategory>('central');
  ...
  // Cancel request modal/logic
  const cancelRequest = (transferId: string) => {
    if (!confirm('Annuler cette demande ?')) return;
    setState((prev) => ({ ...prev, stockTransfers: prev.stockTransfers.map(t => t.id === transferId ? { ...t, status: 'cancelled' as const } : t) }));
  };
  ...
  const pendingRequests = state.stockTransfers.filter((t) => t.status === 'requested' && (reqCatFilter === 'all' || t.category === reqCatFilter));
  ...
  const accentBtn: Record<string, string> = {
    rose: 'bg-rose-500 hover:bg-rose-600 border-rose-600 text-rose-600',
    blue: 'bg-blue-500 hover:bg-blue-600 border-blue-600 text-blue-600',
    purple: 'bg-purple-500 hover:bg-purple-600 border-purple-600 text-purple-600',
    sky: 'bg-sky-500 hover:bg-sky-600 border-sky-600 text-sky-600',
  };
  const selectCls = purchaseCategory === 'hospitalisation' ? 'border-rose-400 focus:ring-rose-500' : purchaseCategory === 'bloc' ? 'border-blue-400 focus:ring-blue-500' : purchaseCategory === 'approvisionnement' ? 'border-purple-400 focus:ring-purple-500' : 'border-sky-400 focus:ring-sky-500';
  const headerBg = purchaseCategory === 'hospitalisation' ? 'bg-rose-50 border-rose-200 text-rose-800' : purchaseCategory === 'bloc' ? 'bg-blue-50 border-blue-200 text-blue-800' : purchaseCategory === 'approvisionnement' ? 'bg-purple-50 border-purple-200 text-purple-800' : 'bg-sky-50 border-sky-200 text-sky-800';
  const selRowBg = purchaseCategory === 'hospitalisation' ? 'bg-rose-500' : purchaseCategory === 'bloc' ? 'bg-blue-500' : purchaseCategory === 'approvisionnement' ? 'bg-purple-500' : 'bg-sky-500';
  ...
  const validatePurchase = () => {
    ...
    setState((prev) => {
      ...
      purchaseLines.forEach((line) => {
        nextArticles = nextArticles.map((a) => {
          if (a.id === line.articleId) {
            // Approvisionnement = pour pharmacie ; central/hospit/bloc = stock central
            if (purchaseCategory === 'approvisionnement') {
              return { ...a, stockPharmacie: a.stockPharmacie + line.quantity, purchasePrice: line.purchasePrice, expiryDate: line.expiryDate || a.expiryDate, supplier: supplier.trim() };
            }
            return { ...a, stockCentral: a.stockCentral + line.quantity, purchasePrice: line.purchasePrice, expiryDate: line.expiryDate || a.expiryDate, supplier: supplier.trim() };
          }
          return a;
        });
      ...
      const next = { ...prev, articles: nextArticles, stockEntries: [...prev.stockEntries, ...newEntries] };
      addAuditLog(next, 'ACHAT_STOCK', `[${transferCategoryLabel(purchaseCategory)}] Achat ${purchaseLines.length} articles chez ${supplier.trim()} (BL: ${invoiceRef.trim()})`);
      return next;
    });
    ...
    setInvoiceRef('');
    purchaseNew();
    alert(`Achat (${transferCategoryLabel(purchaseCategory)}) validé !`);
  };
  ...
  const transferToPharmacy = (transferId: string) => {
    ...
    if (tr.category === 'approvisionnement') {
      if (art.stockCentral < tr.quantity) { alert(`Stock central insuffisant (${art.stockCentral})`); return; }
      setState((prev) => ({
        ...prev,
      ...
      addNotification(state, 'pharmacy', `📦 Réappro reçu: ${tr.articleName} (${tr.quantity})`, 'info');
    } else {
      // Achats hospit/bloc/central demandes - just mark transferred (no pharmacy transfer)
      setState((prev) => ({
        ...prev,
        stockTransfers: prev.stockTransfers.map((t) => t.id === transferId ? { ...t, status: 'transferred' as const, transferredBy: prev.currentUser?.id, transferredAt: new Date().toISOString() } : t),
      }));
      const target = tr.category === 'hospitalisation' ? 'hospitalization' : tr.category === 'bloc' ? 'hospitalization' : 'pharmacy';
      addNotification(state, target as any, `✅ Demande traitée: ${tr.articleName} (${tr.quantity}) — ${transferCategoryLabel(tr.category)}`, 'info');
    }
  };

  const entriesFiltered = state.stockEntries.filter(e => entryCatFilter === 'all' || (e.category || 'central') === entryCatFilter);
  const categoryCounts = TRANSFER_CATEGORIES.reduce<Record<string, number>>((acc, c) => {
    acc[c] = state.stockEntries.filter(e => (e.category || 'central') === c).length;
    return acc;
  }, {});
  const reqCounts = TRANSFER_CATEGORIES.reduce<Record<string, number>>((acc, c) => {
    acc[c] = state.stockTransfers.filter(t => t.status === 'requested' && t.category === c).length;
    return acc;
  }, {});
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <div className="flex border-b overflow-x-auto">
          {[
            ...
            { key: 'entries' as Tab, l: `📥 Entrées / Achats (${state.stockEntries.length})` },
        ...
        <div className="p-4">
          ...
          {tab === 'entries' && (
            <div className="space-y-4">
              <div className={`p-4 ${headerBg.split(' ').slice(0,2).join(' ')} border rounded-xl`}>
                <h4 className={`font-bold flex items-center gap-2 mb-3 ${headerBg.split(' ').slice(2).join(' ')}`}>
                  <PackagePlus className="w-5 h-5" /> Gestionnaire d'Achats & Entrées Stock (Saisie Sage) — {transferCategoryLabel(purchaseCategory)}
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-1">
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => setPurchaseCategory(e.target.value as TransferCategory)}
                      className={`w-full px-3 py-1.5 border rounded-lg outline-none focus:ring-2 bg-white text-sm text-slate-800 ${selectCls}`}
                    >
                      {TRANSFER_CATEGORIES.map(c => <option key={c} value={c}>{transferCategoryLabel(c)}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">👤 Fournisseur *</label>
                    <input type="text" value={supplier} onChange={e => setSupplier(e.target.value)} className="w-full px-3 py-1.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm text-slate-800" placeholder="Nom du fournisseur..." />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">📄 N° BL / Facture *</label>
                    <input type="text" value={invoiceRef} onChange={e => setInvoiceRef(e.target.value)} className="w-full px-3 py-1.5 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm text-slate-800" placeholder="Ex: BL-2026-004..." />
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {TRANSFER_CATEGORIES.map(c => (
                  <button key={c} onClick={() => setPurchaseCategory(c)} className={`px-3 py-1 rounded-full text-xs font-semibold cursor-pointer border transition ${purchaseCategory===c?`${transferCategoryColor(c)} border-transparent shadow`:'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                    {transferCategoryLabel(c)} <span className="opacity-60">({categoryCounts[c] || 0})</span>
                  </button>
                ))}
              </div>
              ...
              <div className="bg-[#f4f4f4] border border-slate-300 rounded text-xs select-none p-1">
                <div className="bg-slate-100 border-b border-slate-300 p-2 m-1.5 mb-0 rounded shadow-inner">
                  <div className="flex flex-wrap items-end gap-1.5 font-sans">
                    <div className="flex-1 min-w-[150px] relative">
                      ...
                      <input
                        ...
                        onChange={e => { setPurchaseSearch(e.target.value); setPurchaseSearchIdx(0); if (purchaseForm.articleName && e.target.value !== purchaseForm.articleName) setPurchaseForm(prev => ({ ...prev, articleName: '', articleId: '' })); }}
                        onKeyDown={purchaseArtKeyDown}
                        className={`w-full bg-white border rounded px-1.5 py-0.5 text-xs font-mono outline-none focus:ring-1 text-slate-800 ${selectCls}`}
                      ...
                      {purchaseSearch.length >= 1 && purchaseFiltered.length > 0 && (
                        <div className="absolute top-full left-0 right-0 bg-white border border-slate-300 rounded-b shadow-2xl z-40 max-h-40 overflow-y-auto">
                          {purchaseFiltered.map((a, idx) => (
                            <div key={a.id} onClick={() => purchaseSelectArticle(a.id)} className={`px-3 py-1.5 cursor-pointer text-xs flex justify-between border-b border-slate-100 ${idx === purchaseSearchIdx ? `${selRowBg} text-white font-medium` : 'hover:bg-slate-50 text-slate-800'}`}>
                              <span>[{familyLabel(a.family)}] {a.name}</span>
                              <span className={`font-mono ${idx === purchaseSearchIdx ? 'text-white' : 'text-slate-500'}`}>Central: {a.stockCentral} / Pharma: {a.stockPharmacie}</span>
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  <div className="flex justify-end gap-1.5 mt-2">
                    ...
                    <button onClick={purchaseSaveLine} disabled={!purchaseForm.articleName} className={`flex items-center gap-1 px-2.5 py-1 text-white border rounded shadow-sm font-semibold disabled:opacity-40 transition cursor-pointer text-xs ${accentBtn[purchaseCategory==='hospitalisation'?'rose':purchaseCategory==='bloc'?'blue':purchaseCategory==='approvisionnement'?'purple':'sky']}`}><Save className="h-3.5 w-3.5" /> Enregistrer</button>
                  </div>
                </div>
                  ...
                  <table className="w-full text-[11px] text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-150 font-mono">
                      {purchaseLines.map(l => {
                        ...
                        return (
                          <tr key={l.id} onClick={() => { setPurchaseSelLineId(l.id); setPurchaseForm({ ...l }); setPurchaseIsNew(false); }} className={`cursor-pointer divide-x divide-slate-200 transition-colors ${isSel ? `${selRowBg} text-white font-medium` : 'hover:bg-slate-50 text-slate-800'}`}>
                    ...
                    {purchaseLines.length > 0 && (
                      <tfoot className="bg-emerald-50 border-t-2 border-emerald-300 text-slate-800 font-sans font-bold">
                        <tr><td colSpan={5} className="p-1.5 text-right text-xs">TOTAL ({transferCategoryLabel(purchaseCategory)}) :</td><td colSpan={2} className="p-1.5 text-right font-mono text-lg text-emerald-700">{formatAr(purchaseLines.reduce((s, l) => s + l.amount, 0))}</td></tr>
                      </tfoot>
                    )}
                  </table>
                </div>
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button onClick={validatePurchase} disabled={purchaseLines.length === 0 || !supplier.trim() || !invoiceRef.trim()} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold disabled:opacity-40 cursor-pointer flex items-center gap-2 shadow transition"><Check className="w-4 h-4" /> Valider {transferCategoryLabel(purchaseCategory)}</button>
              </div>

              {/* Purchase History */}
              <div className="border-t pt-4">
                <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                  <h5 className="font-bold text-sm text-slate-700">📜 Historique des entrées d'achats</h5>
                  <div className="flex items-center gap-1 flex-wrap">
                    ...
                    {TRANSFER_CATEGORIES.map(c => (
                      <button key={c} onClick={() => setEntryCatFilter(c)} className={`px-2 py-0.5 rounded text-xs cursor-pointer ${entryCatFilter===c?'bg-slate-700 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{transferCategoryLabel(c)} ({categoryCounts[c] || 0})</button>
                    ))}
                  </div>
                </div>
                {entriesFiltered.length > 0 ? (
                  <div className="border rounded-lg overflow-hidden bg-white shadow-sm">
                    <table className="w-full text-xs text-left">
                      <thead className="bg-slate-50 border-b"><tr><th className="p-2">Date</th><th className="p-2">Catégorie</th><th className="p-2">Article</th><th className="p-2 text-center">Qté</th><th className="p-2 text-right">P. Achat</th><th className="p-2">Fournisseur</th><th className="p-2">Réf BL</th></tr></thead>
                      <tbody>
                        {entriesFiltered.slice(-30).reverse().map((e) => (
                          <tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td className="p-2"><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${transferCategoryColor((e.category || 'central') as TransferCategory)}`}>{transferCategoryLabel((e.category || 'central') as TransferCategory)}</span></td>
          ...
          {tab === 'requests' && (
            <div>
              <div className="flex items-center gap-2 flex-wrap mb-3">
                <Filter className="w-4 h-4 text-slate-400" />
                <button onClick={() => setReqCatFilter('all')} className={`px-2 py-0.5 rounded text-xs cursor-pointer ${reqCatFilter==='all'?'bg-slate-700 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>Toutes ({state.stockTransfers.filter(t=>t.status==='requested').length})</button>
                {TRANSFER_CATEGORIES.map(c => (
                  <button key={c} onClick={() => setReqCatFilter(c)} className={`px-2 py-0.5 rounded text-xs cursor-pointer ${reqCatFilter===c?'bg-slate-700 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>{transferCategoryLabel(c)} ({reqCounts[c] || 0})</button>
                ))}
              </div>
              ...
              {pendingRequests.length === 0 ? (
                ...
                <div className="space-y-2">
                  {pendingRequests.map((tr) => {
                    ...
                    const canTransfer = tr.category === 'approvisionnement' ? art && art.stockCentral >= tr.quantity : true;
                    return (
                      <div key={tr.id} className="border rounded-lg p-4 flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => transferToPharmacy(tr.id)}
                            disabled={!canTransfer}
                            className={`px-4 py-2 text-white rounded-lg hover:opacity-90 cursor-pointer flex items-center gap-2 text-sm disabled:opacity-40 ${tr.category === 'hospitalisation' ? 'bg-rose-600' : tr.category === 'bloc' ? 'bg-blue-600' : tr.category === 'approvisionnement' ? 'bg-emerald-600' : 'bg-sky-600'}`}
                          >
                            <Truck className="w-4 h-4" /> {tr.category === 'approvisionnement' ? 'Transférer' : 'Marquer livré'}
                          </button>
                        </div>
                      </div>
                    );
